...
const ProductDetails = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const res = await axios.get(
          `https://nutrasurge-reviews.onrender.com/api/products/${id}`,
        );
        setProduct(res.data);
      } catch (err) {
  ...
  const averageRating = useMemo(() => {
    ...
    const sum = acceptedReviews.reduce(
      (acc, r) => acc + Number(r.starRating || 0),
      0,
    );
  ...
  if (error) {
    return (
      <div
        className="card"
        style={{ maxWidth: 900, margin: "2rem auto", textAlign: "center" }}
      >
